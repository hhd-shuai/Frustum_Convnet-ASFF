...
def train(data_loader, model, loss, optimizer, lr_scheduler, epoch, logger=None):
    ...
    tic = time.time()
    loader_size = len(data_loader)      # 1854

    training_states = TrainingStates()


    # 将 一个object的data_dicts_var 加入到场景列表 data_dicts_var_list 中
    data_dicts_var_list = []

    # 下面几行摘自provider_sample.py的82行，overwritten_data_path应该没什么问题吧
    root_data = r"kitti/data/pickle_data"
    car_only = True
    # split =
    if car_only:
    ...
    with open(overwritten_data_path, 'rb') as fp:
        ...
        print(f"传入trian_net_det的id_list = {id_list}")

    for i, (data_dicts) in enumerate(data_loader):

        if id_list[i + 1] == id_list[i]:        # 如果后面的是同一场景就不急着处理，先加进来再说
            ...
            data_dicts_var_list.append(data_dicts_var)

            continue

        else:                                   # 如果是同一场景，就输入，最后还要把data_dicts_var_list 清零

            optimizer.zero_grad()

            x = model(data_dicts_var_list)      # 将场景列表输入[obj1的data_dicts{}， obj2的data_dicts{}]
            print(x.shape)
            ...
            losses, metrics = loss(data_dicts_var_list, x)      # 注意loss函数没有改
            ...
            if (i + 1) % cfg.disp == 0 or (i + 1) == loader_size: # disp: 100
...
def train2augment(data_loader, model, loss, optimizer, lr_scheduler, epoch, logger=None):
    """为了增强深度高的背景点？？？，到底要不要这个还两说"""
...
def main():
    ...
    if args.cfg_file is not None:
        merge_cfg_from_file(ROOT_DIR + "/" + args.cfg_file)

    if args.opts is not None:
        str = ','.join(args.opts)
        liststr = str.split("=")
        merge_cfg_from_list(liststr)
    ...
    dataset_def = import_from_file(ROOT_DIR + '/' + cfg.DATA.FILE)      # '/home/b516/hhd_shuai/vscode_file/frustum-convnet',   'datasets/provider_sample.py'
    ...
    model_def = import_from_file(ROOT_DIR + '/'+ cfg.MODEL.FILE)
    model_def = model_def.PointNetDet
    loss_def = import_from_file(ROOT_DIR + '/'+ cfg.MODEL.LOSS)
    loss_def = loss_def.PointNetDetLoss

    input_channels = 3 if not cfg.DATA.WITH_EXTRA_FEAT else cfg.DATA.EXTRA_FEAT_DIM # input_channels = 3
    ...
    NUM_VEC = len(datset_category_info.CLASSES) # rgb category as extra feature vector # NUM_VEC = 3
    NUM_CLASSES = cfg.MODEL.NUM_CLASSES # NUM_CLASSES=2

    model = model_def(input_channels, num_vec=NUM_VEC, num_classes=NUM_CLASSES)
    loss = loss_def(num_classes=NUM_CLASSES)
    ...
    for n in range(start_epoch, MAX_EPOCH):
        ...
        # 将epoch分成2次训练
        # 第1次，当 n < MAX_EPOCH,目的是为了拿到比较准确的z值的特征信息？
        if n < 0.7 * MAX_EPOCH:
            train(train_loader, model, loss, optimizer, lr_scheduler, n, logger_train)
        # 第2次，目的是增强小目标？？？
        else:
            train(train_loader, model, loss, optimizer, lr_scheduler, n, logger_train)
