...
class ConvFeatNet(nn.Module):
    def __init__(self, i_c=128, num_vec=3):
        ...
        # self.levels_weight = ASFF.add_conv1d(768, 8, 1, 1)
        self.levels_weight = ASFF.add_conv1d(256, 8, 1, 1)
        # self.levels_weight = ASFF.add_conv1d(256, 16, 1, 1)

        self.expand = ASFF.add_conv1d(256, 768, 1, 1)

        self.levels_weights = nn.Conv1d(8*3, 3, kernel_size=1, stride=1, padding=0)

        # self.asff_block = ASFF()
    ...
    def forward(self, x1, x2, x3, x4):
        ...
        # torch.Size([32, 768, 140])
        levels_weight1 = self.levels_weight(xx1)
        #print(f'levels_weight1.shape {levels_weight1.shape}')
        # torch.Size([32, 8, 140])
        levels_weight2 = self.levels_weight(xx2)
        #print(f'levels_weight2.shape {levels_weight2.shape}')
        levels_weight3 = self.levels_weight(xx3)
        #print(f'levels_weight3.shape {levels_weight3.shape}')

        levels_weight_v = torch.cat((levels_weight1, levels_weight2, levels_weight3), 1)
        levels_weights = self.levels_weights(levels_weight_v)
        levels_weights = F.softmax(levels_weights, dim=1)
        # print(f"levels_weights {levels_weights.shape}")
        # print(levels_weights.shape)

        fused_out_reduced = xx1 * levels_weights[:, 0:1, :] +\
                            xx2 * levels_weights[:, 1:2, :] +\
                            xx3 * levels_weights[:, 2:, :]
...
class PointNetDet(nn.Module):
    ...
    def forward(self,
        ...
        ###这里保存的是哪一个远近物体的特征特征 tensor to numpy##
        ####同一个场景里面是否是最后一个物体 保存（,id+x）tensor能不能再cuda保存
