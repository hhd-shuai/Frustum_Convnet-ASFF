...
class PointNetDet(nn.Module):
    ...
    def forward(self,
        ...
        :param scene_data_dicts:   原来：单个obj的信息字典，修改后：一个场景的所有obj的信息字典的列表[obj1_dict, obj2_dict, ...]
        :return:
        """
        scene_x = []
        for data_dicts in scene_data_dicts:
            ...
            center_label = data_dicts.get('box3d_center')       # 要使用box3d_center的中z的距离（刚开始训练时z值与实际偏差很远）
            ...
            obj_x = self.conv_net(feat1, feat2, feat3, feat4)

            # # 此处加一个判断该特征距离的语句（特征obj_x都不变）
            # if center_label[2] >= 50:
            #     random_sample = np.random.random_sample
            #     center_label2 = [center_label[0] + 5*random_sample(), center_label[1] + 5*random_sample(),
            #                      20*random_sample() + 50]     # 为距离远的物体复制一个（但是center_label即box3d不同）
            #     center_label3 = [center_label[0] + 5*random_sample(), center_label[1] + 5*random_sample(),
            #                      20 * random_sample() + 50]  # 为距离远的物体再复制一个（但是center_label即box3d不同）
            ...
            scene_x.append(obj_x)

        ###这里保存的是哪一个远近物体的特征特征 tensor to numpy##

        x = torch.Tensor(scene_x)
