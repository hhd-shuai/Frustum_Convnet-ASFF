...
def extract_frustum_data(idx_filename, split, output_filename,
    ...
    '''
    dataset = kitti_object(os.path.join(ROOT_DIR, 'data/kitti'), split)
    data_idx_list = [int(line.rstrip()) for line in open(idx_filename)] # data_idx是从idx_filename中读取的每一行，不连续，或许是随机的
    ...
    for data_idx in data_idx_list:
        ...
        pc_rect[:, 0:3] = calib.project_velo_to_rect(pc_velo[:, 0:3])
        pc_rect[:, 3] = pc_velo[:, 3]
        img = dataset.get_image(data_idx)       # 获取第idx的图像数据，shape=(370,1224,3)
        img_height, img_width, img_channel = img.shape
        _, pc_image_coord, img_fov_inds = get_lidar_in_image_fov(pc_velo[:, 0:3],
                                                                 calib, 0, 0, img_width, img_height, True)
        # 循环遍历对象，如果该对象的标签的种类在type_white列表
        for obj_idx in range(len(objects)):
...
def get_box3d_dim_statistics(idx_filename):
    ...
    dimension_list = []
    type_list = []
    # 训练的索引列表data_idx_list从train.txt中逐行读取
    data_idx_list = [int(line.rstrip()) for line in open(idx_filename)]
    for data_idx in data_idx_list:
        print('------------- ', data_idx)
        calib = dataset.get_calibration(data_idx)  # 3 by 4 matrix   实际上没有使用该变量
        objects = dataset.get_label_objects(data_idx)
        for obj_idx in range(len(objects)):
            obj = objects[obj_idx]
...
def read_det_file(det_filename):
    ...
    ''' 解析2D detection 输出的 非pkl类型文件的每一行 '''
    det_id2str = {1: 'Pedestrian', 2: 'Car', 3: 'Cyclist'} # default definition in rgb_detection_train/val.txt by rqi
    id_list = []
    type_list = []
    prob_list = []
    box2d_list = []
    # /home/rqi/Data/KITTI/object/training/image_2/000000.png 1 0.999559 718 141 807 311
    for line in open(det_filename, 'r'):
        t = line.rstrip().split(" ")
        id_list.append(int(os.path.basename(t[0]).rstrip('.png')))  # os.path.basename(s[0]) = ''000000.png''
        try:
            cls_type = det_id2str[int(t[1])]     # 1，2，3这种再转为Pedestrian这些
        except ValueError:
            assert t[1] in det_id2str.values()
            cls_type = t[1]
        type_list.append(cls_type)
        prob_list.append(float(t[2]))            # 概率列表，似乎没有用到
        box2d_list.append(np.array([float(t[i]) for i in range(3, 7)])) # box2D的2个坐标（4个数值）
    return id_list, type_list, box2d_list, prob_list


def read_det_pkl_file(det_filename):
    ''' Parse lines in 2D detection output files '''
    ''' 解析2D detection 输出的pkl类型文件的每一行 '''
...
def extract_frustum_data_rgb_detection(det_filename, split, output_filename,
    ...
    ''' Extract point clouds in frustums extruded from 2D detection boxes.
        Update: Lidar points and 3d boxes are in *rect camera* coord system
            (as that in 3d box label files)
            从二维detection box中提取视锥中的点云。
        更新:激光雷达点和3d盒子在*rect相机* coord系统 (如3d box标签文件)
    ...
    id_list = []
    type_list = []
    box2d_list = []     # 相比extract_frustum_data函数没有box3d_list
    prob_list = []
    input_list = []  # channel number = 4, xyz,intensity in rect camera coord
    frustum_angle_list = []  # angle of 2d box center from pos x-axis
    calib_list = []
    ...
    for det_idx in range(len(det_id_list)):
        data_idx = det_id_list[det_idx]         # det_idx是对象索引，即1,2,3,4....data_idx是场景索引，如[1,1,2,2,2,3,3,...]
        ...
        # 2D BOX: Get pts rect backprojected
        det_box2d = det_box2d_list[det_idx].copy()  # [389. 181. 424. 202.]
        det_box2d[[0, 2]] = np.clip(det_box2d[[0, 2]], 0, img_width - 1)        # 截断函数
        ...
        box_fov_inds = (pc_image_coord[:, 0] < xmax) & \
                       ...
                       (pc_image_coord[:, 1] < ymax) & \
                       (pc_image_coord[:, 1] >= ymin)
        box_fov_inds = box_fov_inds & img_fov_inds              # 究竟是box框溢出了图片的意思，还是？？
        pc_in_box_fov = pc_rect[box_fov_inds, :]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_trainval', action='store_true',
                        help='Generate trainval split frustum data with perturbed GT 2D '
                             '')
    ...
    if args.gen_avg_dim:    # 获取box3D，训练的索引从train.txt中逐行读取（）
